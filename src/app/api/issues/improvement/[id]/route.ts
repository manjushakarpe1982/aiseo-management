...
export async function PATCH(
  ...
  try {
    ...
    const current = await db.request()
      .input('id', sql.Int, id)
      .query(`SELECT Status, UserComment, PageURL FROM ClCode_ContentImprovements WHERE ImprovementID = @id`);
    ...
    await db.request()
      ...
      .query(`
        UPDATE ClCode_ContentImprovements
        SET
          Status = @status,
          UserComment = @comment,
    ...
    await db.request()
      .input('entityType', sql.NVarChar, 'ContentImprovement')
      .input('entityID', sql.Int, id)
      .input('entityURL', sql.NVarChar, old.PageURL ?? null)
    ...
    console.error('improvement PATCH error:', err);
